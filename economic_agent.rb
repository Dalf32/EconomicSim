...
class EconomicAgent
  ...
  def trade_results(commodity, amount, price)
    ...
    if @outstanding_asks[commodity] != nil
      ...
      amount = -amount
      @outstanding_asks[commodity] = nil unless !ask.fulfilled?
      ...
      @outstanding_bids[commodity] = nil unless !bid.fulfilled?
    end
  ...
  def update_failed_trades
    @market.commodities.each{|commodity|
  ...
  def generate_asks
    @role.commodities_to_sell.each{|commodity|
  ...
  def generate_bids
    @role.commodities_to_buy.each{|commodity|
